...
export class HeatMeter extends Container {
  ...
  constructor(width: number, height = 14) {
    super();
    ...
    this.txt = new Text({ text: 'HEAT ×1', style: {
      fill: THEME.amber2, fontFamily: 'system-ui, sans-serif', fontSize: Math.round(height * 0.95), fontWeight: '700' as const } });
    this.txt.position.set(0, -height - 4);
    this.addChild(this.track, this.fill, this.txt);
  ...
  reset(heat = 1): void { this.cur = heat; this.txt.text = `HEAT ×${heat}`; this.redraw(heat / this.cap); }
}
